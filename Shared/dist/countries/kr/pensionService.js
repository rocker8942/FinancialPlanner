...
function getContributionYears(eligibilityAge) {
    return Math.min(NPS_FULL_CONTRIBUTION_YEARS, Math.max(NPS_MIN_CONTRIBUTION_YEARS, eligibilityAge - 25));
}
...
const BASIC_PENSION_MAX_ANNUAL_KRW = BASIC_PENSION_MAX_MONTHLY_KRW * 12; // 4,196,400
...
function estimateNpsComponents(individualAnnualSalary, age, birthYear) {
    ...
    // B값: 기준소득월액 상한/하한 클램프
    const bValue = Math.min(NPS_INCOME_CEILING_ANNUAL_KRW, Math.max(NPS_INCOME_FLOOR_ANNUAL_KRW, individualAnnualSalary));
...
function estimateBasicPension(age, financialAssets, isCouple, npsTotalMonthly, // NPS 총 월수령액 (연계감액 발동 조건 ①)
npsAComponentMonthly, // A급여 월액 (연계감액 발동 조건 ②)
cpiAdjustmentFactor) {
...
export function getKoreanPensionAmounts(input) {
    ...
    const userBasic = estimateBasicPension(userAge, financialAssets, applyCoupleFactor, userNpsMonthly, userNps.aComponentMonthly, cpiAdjustmentFactor);
    const partnerBasic = partnerBasicEligible
        ? estimateBasicPension(partnerAge, financialAssets, applyCoupleFactor, partnerNpsMonthly, partnerNps.aComponentMonthly, cpiAdjustmentFactor)
        : 0;
    return {
